...
resize_hash(kissat *solver, checker *checker)
{
  ...
  line **table = (line **)kissat_calloc(solver, new_hashed, sizeof(line *));
...
{
  line *res = (line *)kissat_malloc(solver, bytes_line(size));
  res->next = 0;
  res->size = size;
  res->hash = hash;
...
insert_imported(kissat *solver, checker *checker, unsigned hash)
{
  ...
  if (checker->linesk == checker->hashed)

...
void kissat_add_unchecked_internal(struct kissat *solver, size_t size, unsigned *lits);
...
#define ADD_UNCHECKED_INTERNAL(SIZE, LITS)                   \
  do                                                         \
  {                                                          \
    if (GET_OPTION(check) > 1)                               \
      kissat_add_unchecked_internal(solver, (SIZE), (LITS)); \
  } while (0)

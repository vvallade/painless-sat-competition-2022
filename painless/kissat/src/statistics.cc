...
int get_decision(kissat *solver)
{
  statistics *statistics = &solver->statistics;
  return statistics->decisions;
}

int get_conflict(kissat *solver)
{
  statistics *statistics = &solver->statistics;
  return statistics->conflicts;
}

int get_propagation(kissat *solver)
{
  statistics *statistics = &solver->statistics;
  return statistics->propagations;
}

int get_restart(kissat *solver)
{
  statistics *statistics = &solver->statistics;
...
double get_memory()
{
  return (double)kissat_maximum_resident_set_size() / (1024 * 1024);
}

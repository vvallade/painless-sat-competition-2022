...
void inc_clause(kissat *solver, bool original, bool redundant)
{
  if (redundant)
...
void init_clause(kissat *solver, clause *res,
                 bool redundant, unsigned glue, unsigned size)
{
  ...
  unsigned tier1 = GET_OPTION(tier1);
...
static reference
new_imported_binary_clause(kissat *solver, bool redundant,
                           unsigned first, unsigned second)
{
  assert(first != second);
  assert(first != NOT(second));
  ...
  inc_clause(solver, false, redundant);
  CHECK_AND_ADD_BINARY(first, second);
  ADD_BINARY_TO_PROOF(first, second);
  return INVALID_REF;
}
...
static reference
new_large_imported_clause(kissat *solver, bool redundant, unsigned glue,
                          unsigned size, unsigned *lits)
{
  ...
  init_clause(solver, c, redundant, 0, size);
  c->glue = glue;
...
reference
new_imported_clause(kissat *solver, bool redundant,
                    unsigned glue, unsigned size, unsigned *lits)
{
  reference res;
  if (size == 2)
    res = new_imported_binary_clause(solver, redundant, lits[0], lits[1]);
  else
    res = new_large_imported_clause(solver, redundant, glue, size, lits);
